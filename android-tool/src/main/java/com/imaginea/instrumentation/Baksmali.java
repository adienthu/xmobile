...
public class Baksmali
{
  ...
  public static boolean decompileAndInstrument(String paramString1, int paramInt, String paramString2)
  {
    File localFile = new File(paramString1);
    if (!localFile.exists())
    {
      System.out.println("Can't find the file " + paramString1);
      return false;
    }
    DexBackedDexFile localDexBackedDexFile;
    try
    {
      localDexBackedDexFile = DexFileFactory.loadDexFile(localFile, paramInt);
    }
    catch (IOException localIOException)
    {
      System.out.println("Couldn't load dex file" + paramString1);
    ...
    if ((i != 0) || (localbaksmaliOptions.registerInfo != 0))
      if ((localDexBackedDexFile instanceof DexBackedOdexFile))
        ...
        localbaksmaliOptions.bootClassPathEntries = getDefaultBootClassPathForApi(paramInt);
